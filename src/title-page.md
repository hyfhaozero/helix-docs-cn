# Helix

<div class="version-tiles">
  <a class="version-tile" href="/">
    <strong>稳定版(Stable)</strong>
    <span class="version-tile-sub">最新正式版(tagged release)的文档</span>
  </a>
  <a class="version-tile" href="/master/">
    <strong>最新版(Master)</strong>
    <span class="version-tile-sub">基于 <code>master</code> 分支构建的最新开发版文档.</span>
  </a>
</div>

有关编辑器的快速概览，请参阅 [usage]（使用）章节；有关所有可用键位绑定，请参阅 [keymap]（键位映射）章节；有关定义自定义键位绑定、设置主题等, 请参阅 [configuration]（配置）章节.
其他内容（例如如何安装支持的语言服务器），请参阅 [Helix Wiki].

常见问题请参考 [FAQ]（常见问题）.

[常见问题(FAQ)]: https://github.com/helix-editor/helix/wiki/FAQ
[使用]: ./usage.md
[按键映射]: ./keymap.md
[配置]: ./configuration.md
[Helix Wiki]: https://github.com/helix-editor/helix/wiki